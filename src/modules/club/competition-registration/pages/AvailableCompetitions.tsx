/**
 * AvailableCompetitions - Konsumsi dari shared CompetitionDataContext
 */

import { useState, useMemo } from "react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import {
  Select, SelectContent, SelectItem, SelectTrigger, SelectValue,
} from "@/components/ui/select";
import { CompetitionCard } from "../components/CompetitionCard";
import { useNavigate } from "react-router-dom";
import { Search, Grid2X2, List } from "lucide-react";
import { cn } from "@/lib/utils";
import { useCompetitionData } from "@/context/CompetitionDataContext";

export default function AvailableCompetitions() {
  const navigate = useNavigate();
  const { competitions, isClubRegistered } = useCompetitionData();
  const [searchQuery, setSearchQuery] = useState("");
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");
  const [filterStatus, setFilterStatus] = useState<string>("all");
  const [filterAgeGroup, setFilterAgeGroup] = useState<string>("all");
  const [filterFormat, setFilterFormat] = useState<string>("all");

  const ageGroups = useMemo(() => {
    return Array.from(new Set(competitions.map(c => c.ageGroup))).sort();
  }, [competitions]);

  const formats = useMemo(() => {
    return Array.from(new Set(competitions.map(c => c.format))).sort();
  }, [competitions]);

  const statusMap: Record<string, string> = {
    'Draft': 'Draft',
    'Registration Open': 'Registration Open',
    'Active': 'Active',
    'Finished': 'Finished',
  };

  const filteredCompetitions = useMemo(() => {
    return competitions.filter(comp => {
      if (searchQuery.trim()) {
        const query = searchQuery.toLowerCase();
        if (!comp.name.toLowerCase().includes(query) &&
            !comp.eoName.toLowerCase().includes(query) &&
            !comp.ageGroup.toLowerCase().includes(query)) {
          return false;
        }
      }
      if (filterStatus !== "all" && comp.status !== filterStatus) return false;
      if (filterAgeGroup !== "all" && comp.ageGroup !== filterAgeGroup) return false;
      if (filterFormat !== "all" && comp.format !== filterFormat) return false;
      return true;
    });
  }, [competitions, searchQuery, filterStatus, filterAgeGroup, filterFormat]);

  const stats = {
    total: competitions.length,
    open: competitions.filter(c => c.status === "Registration Open").length,
    registered: competitions.filter(c => isClubRegistered(c.id, 'club-1')).length,
  };

  return (
    <div className="space-y-6 animate-fade-in" role="main" aria-label="Available competitions">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Daftar Kompetisi</h1>
          <p className="text-muted-foreground mt-1">
            {stats.total} kompetisi tersedia • {stats.open} pendaftaran dibuka • {stats.registered} terdaftar
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant={viewMode === "grid" ? "default" : "outline"} size="sm" onClick={() => setViewMode("grid")} className="gap-2">
            <Grid2X2 className="w-4 h-4" /><span className="hidden sm:inline">Grid</span>
          </Button>
          <Button variant={viewMode === "list" ? "default" : "outline"} size="sm" onClick={() => setViewMode("list")} className="gap-2">
            <List className="w-4 h-4" /><span className="hidden sm:inline">List</span>
          </Button>
        </div>
      </div>

      {/* Quick Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card className="bg-gradient-to-br from-primary/5 to-primary/10">
          <CardContent className="pt-6 text-center">
            <div className="text-2xl font-bold text-primary">{stats.total}</div>
            <div className="text-xs text-muted-foreground mt-1">Total Kompetisi</div>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-green-500/5 to-green-500/10">
          <CardContent className="pt-6 text-center">
            <div className="text-2xl font-bold text-green-700">{stats.open}</div>
            <div className="text-xs text-muted-foreground mt-1">Pendaftaran Dibuka</div>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-gold/5 to-gold/10">
          <CardContent className="pt-6 text-center">
            <div className="text-2xl font-bold text-gold-foreground">{stats.registered}</div>
            <div className="text-xs text-muted-foreground mt-1">Terdaftar</div>
          </CardContent>
        </Card>
      </div>

      {/* Filters */}
      <Card className="bg-muted/50 border-muted">
        <CardContent className="pt-6 space-y-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input placeholder="Cari kompetisi, EO, atau usia..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger><SelectValue placeholder="Status" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Semua Status</SelectItem>
                <SelectItem value="Draft">Draft</SelectItem>
                <SelectItem value="Registration Open">Pendaftaran Dibuka</SelectItem>
                <SelectItem value="Active">Berlangsung</SelectItem>
                <SelectItem value="Finished">Selesai</SelectItem>
              </SelectContent>
            </Select>
            <Select value={filterAgeGroup} onValueChange={setFilterAgeGroup}>
              <SelectTrigger><SelectValue placeholder="Kelompok Usia" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Semua Usia</SelectItem>
                {ageGroups.map(ag => <SelectItem key={ag} value={ag}>{ag}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={filterFormat} onValueChange={setFilterFormat}>
              <SelectTrigger><SelectValue placeholder="Format" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Semua Format</SelectItem>
                {formats.map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        </CardContent>
      </Card>

      {/* Results */}
      {filteredCompetitions.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-lg font-semibold text-muted-foreground">Tidak ada kompetisi</p>
          <p className="text-sm text-muted-foreground">Coba ubah filter untuk menemukan kompetisi</p>
        </Card>
      ) : (
        <div className={cn(
          viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" : "space-y-3"
        )}>
          {filteredCompetitions.map(competition => (
            <CompetitionCard
              key={competition.id}
              competition={{
                ...competition,
                documents: competition.documents.map(d => ({
                  ...d,
                  acceptedFormats: [],
                  maxFileSize: 5 * 1024 * 1024,
                })),
              } as any}
              onViewDetail={(id) => navigate(`/club/competition/${id}`)}
              onRegister={(id) => navigate(`/club/competition/${id}/register`)}
              isRegistered={isClubRegistered(competition.id, 'club-1')}
              isRegistrationOpen={competition.status === "Registration Open"}
            />
          ))}
        </div>
      )}
    </div>
  );
}
